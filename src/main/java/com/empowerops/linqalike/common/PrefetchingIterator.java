...
public abstract class PrefetchingIterator<TElement> implements Iterator<TElement> {
    ...
    /**
     * <p>Method to retrieve the next value (eagerly) that will be yielded to {@link #next()}. Use
     ...
     * {@link com.empowerops.linqalike.Queryable} interface-contract is concerned, <tt>null</tt> elements are always legal,
     * so we cant simply have the {@link #prefetch()} method return null as a signal to indicate that <i>I cant prefetch
     * another value --we're out.</i> So, instead, we use this method.
     *
     ...
     * and {@link #hasPrefetchedValue()} will once again be false (ie, the prefetch cache will be invalidated).
     *
     * <p>You may not invoke <tt>setPrefetchedValue(Object prefetchedValue)</tt> twice without a call to
     * {@link #next()} between them.
     *
